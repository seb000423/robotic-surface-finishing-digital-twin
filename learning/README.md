...
v5 힘 로그 ──▶ bc/ (모방학습)
                        │
digital_twin/ (표면·제거·GU 모델, BO) ──▶ rl/ (Isaac Lab env · BC 부트스트랩 · PPO)
                                              │
                                              ▼
...
| `rl/` | BC 부트스트랩, PPO 학습(`train_ppo*.py`), 짝지은 평가(`eval_conditions*.py`), 레시피 BO outer loop, 재폴리싱 평가, 차체 셀 순회(`car_cells_robot.py`) |
| `rl/champion/` | 최종 체크포인트 (해석식 env: `model_terminal_ppo_14ch_it800.pt`) |
...
| 보상 | 스텝: 접촉력 안전 · 행동 급변 억제 · 클리어코트 과다 제거 방지 · 결함 제거 / 종료: 전·후 GU · 스크래치 · Ra · Rz 개선량, 판정 통과 보너스, 클리어코트 한계 위반 페널티 |
| 안전 | hard limit 14 N (정책 출력과 무관하게 최종 단에서 적용), 클리어코트 잔여 ≥ 35 µm |
| 알고리즘 | rsl_rl PPO, BC로 actor 초기화 |
...
| 손상차 시나리오 150셀 (자세별 BO 레시피) | 91 → 97 / 150 |
| 잔존 스크래치 (BC 정책, 고정 레시피 대비) | −30 % |
